...
def get_all_scan_functions(credentials, regions=None):
    ...
    try:
        ...
        # --- REVERTED TO TEMP FILE METHOD FOR DEBUGGING ---
        key_file_path = "temp_gcp_key.json"
        with open(key_file_path, "w") as key_file:
            json.dump(key_data, key_file)
        os.environ["GOOGLE_APPLICATION_CREDENTIALS"] = key_file_path
        
        gcp_credentials = service_account.Credentials.from_service_account_file(key_file_path)
        ...
        logging.critical(f"--- FAILED TO INITIALIZE GCP SCANNER ---")
        raise e
