...
class LicenseManager:
    ...
    def validate_license_key(self, license_key: str) -> dict:
        ...
        try:
            ...
            remaining_days = (expiry_date - datetime.utcnow()).days if not is_expired else 0
            ...
            if is_expired:
                result['error'] = f'License expired on {expiry_date.strftime("%Y-%m-%d")}'
                logger.warning(f"Expired license key used: {license_data['email']}")
            else:
